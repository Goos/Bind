...
// TODO: Add more permutations for this
public func >>= <A: AsyncCallable, B, C, D> (_ first: @escaping (B) throws -> C, _ second: @escaping (C) -> A) -> (B, Callback<D>, ErrorCallback) -> () where A.ReturnType == D {
    return { (b: B, s: Callback<D>, e: ErrorCallback) -> () in
        do {
            let res = try first(b)
